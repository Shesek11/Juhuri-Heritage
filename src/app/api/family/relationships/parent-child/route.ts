...
export async function POST(request: NextRequest) {
  try {
    ...
    const [parent] = await pool.query('SELECT id FROM family_members WHERE id = ?', [parent_id]) as any[];
    const [child] = await pool.query('SELECT id FROM family_members WHERE id = ?', [child_id]) as any[];
