...
export async function GET(request: NextRequest) {
  try {
    ...
    const [rows] = await pool.query(
      `SELECT de.id, de.hebrew_script, de.hebrew_script as t_hebrew_script, de.part_of_speech,
              CASE
                WHEN de.hebrew_script = ? THEN 100
                WHEN de.hebrew_script = ? THEN 90
                WHEN de.hebrew_script LIKE ? THEN 70
                WHEN de.hebrew_script LIKE ? THEN 60
                WHEN de.hebrew_script LIKE ? THEN 40
                WHEN de.hebrew_script LIKE ? THEN 30
                ELSE 10
              END as score
       FROM dictionary_entries de
       LEFT JOIN dialect_scripts t ON de.id = t.entry_id
       WHERE de.status = 'active'
         AND (de.hebrew_script LIKE ? OR de.hebrew_script LIKE ? OR de.russian_short LIKE ?)
       GROUP BY de.id
       ORDER BY score DESC
       LIMIT ?`,
