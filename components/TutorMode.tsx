        ...
        return (
            <div className="w-full max-w-4xl mx-auto mt-8 h-[600px] flex flex-col bg-[#0d1424]/60 backdrop-blur-xl rounded-2xl shadow-xl overflow-hidden border border-white/10 font-rubik">
                <div className="bg-white/5 border-b border-white/10 backdrop-blur-md p-4 flex text-amber-500 justify-between items-center text-white shadow-md z-10">
                    ...
                    <button onClick={() => setMode('map')} className="text-sm bg-amber-500/10 hover:bg-amber-500/20 text-amber-500 border border-amber-500/20 px-3 py-1 rounded">חזרה למפה</button>
                </div>
                <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-transparent scroll-smooth">
                    ...
                    {messages.map((msg) => (
                        <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                            <div className={`max-w-[80%] rounded-2xl p-4 shadow-sm ${msg.role === 'user' ? 'bg-gradient-to-br from-amber-400 to-orange-500 text-[#050B14] font-medium rounded-br-none shadow-md' : 'bg-white/10 backdrop-blur-md text-white border border-white/10 rounded-bl-none'}`}>
                                {msg.content}
                                {msg.audioText && (
                ...
                <form onSubmit={handleSendMessage} className="p-4 bg-[#0d1424]/60 backdrop-blur-xl border-t border-white/10 flex gap-2">
                    <input type="text" value={input} onChange={(e) => setInput(e.target.value)} placeholder="כתוב הודעה..." className="flex-1 px-4 py-3 bg-slate-100 dark:bg-white/5 border-b border-white/10 backdrop-blur-md rounded-xl outline-none dark:text-white" disabled={loading} />
                    <button type="submit" disabled={loading || !input.trim()} className="p-3 bg-gradient-to-br from-amber-400 to-orange-600 text-[#050B14] rounded-xl hover:shadow-[0_0_15px_rgba(245,158,11,0.4)]"><Send size={20} /></button>
                </form>
            </div>
        );
    }
        ...
        <div className="w-full max-w-4xl mx-auto mt-8 bg-[#0d1424]/60 backdrop-blur-xl rounded-2xl shadow-xl overflow-hidden border border-white/10 font-rubik flex flex-col h-[600px]">
                            ...
                            <div key={unit.id} className="relative group w-full max-w-xs flex justify-center">
                                ...
                                <button
                                    ...
                                    ${locked
                                            ...
                                            : completed
                                                ? 'bg-emerald-500 border-emerald-700 shadow-[0_0_20px_rgba(16,185,129,0.4)] text-[#050B14] hover:scale-110'
                                                : 'bg-gradient-to-br from-amber-400 to-orange-600 border-orange-700 shadow-[0_0_20px_rgba(245,158,11,0.4)] text-[#050B14] hover:scale-110'
                                        }
                                `}
                                >
            ...
            <div className="p-4 bg-[#0d1424]/60 backdrop-blur-xl border-t border-white/10 flex justify-center">
                <button
                    ...
                    className="flex items-center gap-2 bg-gradient-to-r from-amber-500/10 to-orange-500/10 border border-amber-500/20 text-amber-500 px-6 py-3 rounded-xl font-bold hover:bg-amber-500/20 hover:border-amber-500/40 shadow-lg transition-colors"
                >
        ...
        <div className="space-y-6">
            <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                    {dialects.map((d) => (
                        <button key={d.id} onClick={() => setDialect(d.name)} className={`p-3 rounded-lg border text-sm font-medium transition-all text-right ${dialect === d.name ? 'border-amber-500/50 bg-amber-500/10 text-amber-500' : 'border-white/10 hover:border-amber-500/50'}`}>
                            {d.description}
                        </button>
                    ))}
                </div>
            ...
            <div>
                ...
                <div className="flex gap-3">
                    {[{ val: 'Beginner', label: 'מתחיל' }, { val: 'Intermediate', label: 'בינוני' }, { val: 'Advanced', label: 'מתקדם' }].map((opt) => (
                        <button key={opt.val} onClick={() => setLevel(opt.val as ProficiencyLevel)} className={`flex-1 p-3 rounded-lg border text-sm font-medium transition-all ${level === opt.val ? 'border-amber-500/50 bg-amber-500/10 text-amber-500' : 'border-white/10 hover:border-amber-500/50'}`}>
                            {opt.label}
                        </button>
                    ))}
                </div>
