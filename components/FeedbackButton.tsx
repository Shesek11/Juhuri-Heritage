    ...
    <>
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4 bg-black/40" onClick={() => setIsOpen(false)}>
          <div
            className="w-full max-w-md bg-white dark:bg-slate-800 rounded-2xl shadow-2xl overflow-hidden animate-in fade-in slide-in-from-bottom-4"
            onClick={e => e.stopPropagation()}
          >
            {sent ? (
              <div className="p-8 text-center space-y-3">
                <CheckCircle className="mx-auto text-green-500" size={48} />
                <p className="text-lg font-bold text-slate-800 dark:text-slate-200">תודה רבה!</p>
                <p className="text-sm text-slate-500 dark:text-slate-400">ההודעה נשלחה בהצלחה</p>
              </div>
            ) : (
              <form onSubmit={handleSubmit}>
                <div className="flex items-center justify-between p-4 border-b border-slate-100 dark:border-slate-700">
                  <h3 className="font-bold text-slate-800 dark:text-slate-200">יש לכם הצעה? דיווח? שלחו לנו!</h3>
                ...
                <div className="p-4 space-y-4">
                  ...
                  <textarea
                    ...
                    required
                    rows={4}
                    className="w-full px-3 py-2 text-sm rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-900 focus:ring-2 focus:ring-indigo-500 outline-none resize-none"
                    dir="auto"
                    autoFocus
                  />
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 text-sm rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-900 focus:ring-2 focus:ring-indigo-500 outline-none"
                    dir="auto"
                  />
                </div>
