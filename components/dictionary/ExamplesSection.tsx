...
const ExamplesSection: React.FC<ExamplesSectionProps> = ({
  ...
  const [proverbMessage, setProverbMessage] = useState<'success' | 'error' | ''>('');
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-sm uppercase tracking-wider text-slate-400 dark:text-slate-400 font-bold">{t('title')}</h3>
        {entry.id && (
          <button
            type="button"
      ...
      {!hasExamples && enrichmentLoading && (
        <EnrichmentSkeleton label={t('aiSearching')} size="lg" />
      )}

      {!hasExamples && !enrichmentLoading && (
        <p className="text-sm text-slate-400 dark:text-slate-400">{t('empty')}</p>
      )}
      ...
      {communityExamples.length > 0 && (
        <div className="mt-4">
          <p className="text-xs font-bold text-indigo-600 dark:text-indigo-400 mb-2 flex items-center gap-1">
            <Users size={12} /> {t('communityProverbs')}
          </p>
          <div className="space-y-3">
            {communityExamples.map((ex) => (
              <div key={ex.id} className="border-s-4 border-indigo-200 dark:border-indigo-900 ps-4 py-2 bg-indigo-50/50 dark:bg-indigo-900/20 rounded-s-lg">
                ...
                <p className="text-[11px] text-slate-400 mt-1">{t('contribution')}: {ex.user_name}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {showProverbForm && (
        <div className="mt-4 p-4 bg-indigo-50 dark:bg-indigo-900/20 rounded-xl border border-indigo-200 dark:border-indigo-800 space-y-3 animate-in fade-in slide-in-from-top-2">
          <div className="flex items-center justify-between">
            <p className="text-sm font-bold text-indigo-700 dark:text-indigo-300">{t('formTitle')}</p>
            <button type="button" onClick={() => setShowProverbForm(false)} className="text-slate-400 hover:text-slate-600" title={tc('close')}>
              <XIcon size={14} />
            </button>
          </div>
          <input type="text" value={proverbOrigin} onChange={e => setProverbOrigin(e.target.value)} placeholder={t('originPlaceholder')} className="w-full px-3 py-2 text-sm rounded-md border border-slate-200 dark:border-slate-600 bg-[#0d1424]/60 backdrop-blur-xl focus-visible:ring-2 focus-visible:ring-indigo-500 outline-none" dir="auto" />
          <input type="text" value={proverbTranslated} onChange={e => setProverbTranslated(e.target.value)} placeholder={t('translationPlaceholder')} className="w-full px-3 py-2 text-sm rounded-md border border-slate-200 dark:border-slate-600 bg-[#0d1424]/60 backdrop-blur-xl focus-visible:ring-2 focus-visible:ring-indigo-500 outline-none" dir="auto" />
          <input type="text" value={proverbTranslit} onChange={e => setProverbTranslit(e.target.value)} placeholder={t('translitPlaceholder')} className="w-full px-3 py-2 text-sm rounded-md border border-slate-200 dark:border-slate-600 bg-[#0d1424]/60 backdrop-blur-xl focus-visible:ring-2 focus-visible:ring-indigo-500 outline-none" dir="ltr" />
          <div className="flex items-center gap-2">
            ...
            {proverbMessage === 'success' && <span className="text-xs text-green-600">{t('submitSuccess')}</span>}
            {proverbMessage === 'error' && <span className="text-xs text-red-500">{t('submitError')}</span>}
          </div>
        </div>
      )}
    </div>
