    ...
    <div className="w-full max-w-5xl mx-auto px-4 py-12">
      <div className="bg-[#0d1424]/60 backdrop-blur-xl rounded-2xl shadow-lg p-8 md:p-12 border border-white/10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
          {/* Form */}
          <div>
            <h2 className="text-xl font-bold mb-5 text-slate-100">שלחו לנו הודעה</h2>
            <form onSubmit={handleSubmit} className="space-y-5">
              <div>
                <label htmlFor="name" className="block text-sm font-medium text-slate-600 dark:text-slate-300 mb-1.5">שם מלא</label>
                <input type="text" id="name" name="name" value={formData.name} onChange={handleChange} required className={inputClasses} />
              </div>
              <div>
                <label htmlFor="email" className="block text-sm font-medium text-slate-600 dark:text-slate-300 mb-1.5">כתובת אימייל</label>
                <input type="email" id="email" name="email" value={formData.email} onChange={handleChange} required className={inputClasses} />
              </div>
              <div>
                <label htmlFor="subject" className="block text-sm font-medium text-slate-600 dark:text-slate-300 mb-1.5">נושא</label>
                <select id="subject" name="subject" value={formData.subject} onChange={handleChange} required className={inputClasses}>
                  ...
                  <option value="contribution">תרומה לפרויקט</option>
              ...
              <div>
                <label htmlFor="message" className="block text-sm font-medium text-slate-600 dark:text-slate-300 mb-1.5">הודעה</label>
              ...
              >
                {formStatus === 'submitting' ? (
                  <><div className="animate-spin rounded-full h-5 w-5 border-b-2 border-white" /> שולח...</>
                ) : 'שלח הודעה'}
              </button>
              {formStatus === 'success' && <p className="text-center text-emerald-600 dark:text-emerald-400">הודעתכם נשלחה בהצלחה!</p>}
              {formStatus === 'error' && <p className="text-center text-red-600 dark:text-red-400">אירעה שגיאה. אנא נסו שוב.</p>}
            </form>
          </div>

          {/* Contact Info */}
          <div className="bg-white/10/50 p-8 rounded-2xl border border-slate-200 dark:border-slate-600">
            <h2 className="text-xl font-bold mb-5 text-slate-100">פרטי התקשרות</h2>
            <div className="space-y-6">
              <div className="flex items-start gap-4">
                <Mail size={22} className="text-amber-500 flex-shrink-0 mt-1" />
                <div>
                  <h3 className="font-semibold text-slate-100">אימייל</h3>
              ...
              <div className="flex items-start gap-4">
                ...
                <div>
                  <h3 className="font-semibold text-slate-100">טלפון</h3>
                  <p className="text-slate-600 dark:text-slate-300 text-sm">העדיפו פנייה באימייל</p>
                </div>
              </div>
              <div className="flex items-start gap-4">
                <MapPin size={22} className="text-amber-500 flex-shrink-0 mt-1" />
                <div>
                  <h3 className="font-semibold text-slate-100">מיקום</h3>
                  <p className="text-slate-600 dark:text-slate-300 text-sm">ישראל</p>
                </div>
              </div>
            </div>
          </div>
